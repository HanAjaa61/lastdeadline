...
export default class JumpscareScene extends Phaser.Scene {
  ...
  create() {
    ...
    // displaySize dikunci ke canvas — tidak bergantung resolusi video
    // setScale TIDAK dipakai supaya ukuran konsisten saat retry
    const video = this.add.video(width / 2, height / 2, "jumpscareVideo")
    video.setOrigin(0.5)
    video.setDepth(5)

    const applySize = () => {
      video.setDisplaySize(width * 1.15, height * 1.15)
    }
    ...
    // Zoom in mendadak — pakai camera zoom bukan video scale
    // supaya ukuran video tidak terpengaruh
    ...
    const rgbRed  = this.add.rectangle(width / 2 - 6, height / 2, width * 1.3, height * 1.3, 0xff0000, 0.12).setDepth(6).setBlendMode(Phaser.BlendModes.ADD)
    const rgbBlue = this.add.rectangle(width / 2 + 6, height / 2, width * 1.3, height * 1.3, 0x0000ff, 0.12).setDepth(6).setBlendMode(Phaser.BlendModes.ADD)
    this.tweens.add({
